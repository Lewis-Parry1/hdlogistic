...
@pytest.fixture(scope="module")
def reduced_result(full_result):
...
class TestPenalisedLRTTypesAndShapes:
    def test_returns_penalised_lrt_results(self, full_result, reduced_result):
    ...
    def test_statistic_is_float(self, full_result, reduced_result, hd_correction):
        result = penalised_lrt(full_result, reduced_result, hd_correction=hd_correction)
        assert isinstance(result.statistic, float)
        assert np.isfinite(result.statistic)

    @pytest.mark.parametrize("hd_correction", [False, True])
    def test_p_value_is_valid_probability(self, full_result, reduced_result, hd_correction):
        result = penalised_lrt(full_result, reduced_result, hd_correction=hd_correction)
        assert isinstance(result.p_value, float)
        assert 0.0 <= result.p_value <= 1.0

    def test_df_matches_rank_difference(self, full_result, reduced_result):
        result = penalised_lrt(full_result, reduced_result, hd_correction=False)
        assert result.df == full_result.rank - reduced_result.rank
        assert isinstance(result.df, (int, np.integer))

    def test_argument_order_does_not_matter(self, full_result, reduced_result):
    ...
    def test_hd_correction_false_leaves_diagnostics_none(self, full_result, reduced_result):
        result = penalised_lrt(full_result, reduced_result, hd_correction=False)
        assert result.kappa is None
        assert result.se_params is None
        assert result.signal_strength is None

    def test_hd_correction_true_populates_diagnostics(self, full_result, reduced_result):
    ...
    def test_dict_style_access_matches_attributes(self, full_result, reduced_result):
        result = penalised_lrt(full_result, reduced_result, hd_correction=True)
    ...
    def test_summary_returns_string(self, full_result, reduced_result):
        result = penalised_lrt(full_result, reduced_result, hd_correction=True)
        assert isinstance(str(result), str)
        assert isinstance(repr(result), str)

class TestPenalisedLRTValidation:
    def test_raises_on_alpha_mismatch(self, full_result):
        mismatched = MDYPLModel(y=y, x=x[:, :2], alpha=0.1).fit()
        with pytest.raises(ValueError, match="alpha"):
            penalised_lrt(full_result, mismatched)

    def test_raises_on_different_response(self, full_result):
        y_other = y.copy()
...
class TestPenalisedLRTAgainstBrglm2:
    ...
    def test_matches_brglm2_no_hd_correction(self, full_result, reduced_result):
        result = penalised_lrt(full_result, reduced_result, hd_correction=False)
        assert result.statistic == pytest.approx(self.EXPECTED_STATISTIC_NO_HD, rel=1e-6)
        assert result.p_value == pytest.approx(self.EXPECTED_PVALUE_NO_HD, rel=1e-6)

    def test_matches_brglm2_hd_correction(self, full_result, reduced_result):
        result = penalised_lrt(full_result, reduced_result, hd_correction=True)
