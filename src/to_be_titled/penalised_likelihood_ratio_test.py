...
class PenalisedLRTResults:
    ...
    def __str__(self) -> str:
        ...
        header = f"{'':<15}{'Rank':>6}{'Deviance':>12}{'Df':>6}{'Stat':>10}{'Pr(>Chi)':>12}"
        lines.append(header)
        lines.append(
        ...
        if self.hd_correction:
            ...
            lines.append(f"Estimated signal strength (gamma^2) = {self.signal_strength:.4g}")
            if self.se_params is not None:
                mu_b_sigma = ", ".join(f"{v:.4g}" for v in self.se_params[:3])
                lines.append(f"State evolution parameters (mu, b, sigma) = ({mu_b_sigma})")
...
def penalised_lrt(
    ...
    *,
    ...
    if not np.array_equal(fit_1.weights, fit_2.weights):
        raise ValueError("Models must be fit on the exact same frequency weights `weights`")
    ...
    if deviance_difference < -1e-12:  # allow for floating-point noise, not real negativity
        warnings.warn(
